$queries = Get-Content -Path .\tools\geocode_nominatim.ps1 -Raw -ErrorAction Stop -Encoding UTF8
Write-Output 'DEBUG: Loaded geocode_nominatim.ps1 content length=' + $queries.Length
# Instead of parsing the ps1, re-declare queries here for verbosity
$queries = @(
...
Write-Output "INFO: Starting queries count=$($queries.Count)"
$count = 0
foreach ($item in $queries) {
    $count++
    Write-Output "QUERY #$count id=$($item.id) q=$($item.q)"
    ...
    try {
        ...
        Write-Output "HTTP OK for $($item.id)"
    } catch {
        Write-Output "HTTP ERROR for $($item.id): $_"
        $resp = @()
    }
    if ($resp -and $resp.Count -gt 0) {
        $r = $resp[0]
        Write-Output "FOUND: $($r.display_name) => $($r.lat),$($r.lon)"
        $results += [pscustomobject]@{id=$item.id; query=$item.q; display_name=$r.display_name; lat=[double]$r.lat; lon=[double]$r.lon; type=$r.type}
    } else {
        Write-Output "NOT FOUND for $($item.id)"
